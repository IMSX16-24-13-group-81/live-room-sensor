...
bool checksum_is_valid(uint8_t *buf, uint8_t len) {
...
void parse_trajectory_info(uint8_t *buf, uint8_t len) {
    if (len < 9) return;
...
void handle_received_frame() {
    ...
    uint16_t control_command = uart_rx_buf[3] << 8 | uart_rx_buf[4];
    switch (control_command) {
        case TRAJECTORY_INFO_REPORT:
            parse_trajectory_info(uart_rx_buf, uart_rx_buf_head);
            break;
        default:
 ...
 */
void micradar_init() {
    memset(previous_counts, 0, COUNT_AVERAGE_BUFFER_SIZE);
