...
const char *request_base = "POST /api/sensors/report HTTP/1.1\r\n"
...
void perform_post_request(const char *request_content, size_t len) {
    ...
    if (!send_request(pcb, request, len)) {
...
bool resolve_hostname(ip_addr_t *ipaddr) {
    ...
    // Attempt resolution
    cyw43_arch_lwip_begin();
    lwip_err_t lwip_err = dns_gethostbyname(PICOHTTPS_HOSTNAME, ipaddr,
...
bool connect_to_host(ip_addr_t *ipaddr, struct altcp_pcb **pcb) {
    ...
    struct altcp_tls_config *config =
            altcp_tls_create_config_client(ca_cert, LEN(ca_cert));
    cyw43_arch_lwip_end();
    if (!config)
        return false;
    ...
    *pcb = altcp_tls_new(config, IPADDR_TYPE_V4);
    ...
    cyw43_arch_lwip_begin();
    lwip_err_t lwip_err =
            altcp_connect(*pcb, ipaddr, LWIP_IANA_PORT_HTTPS, callback_altcp_connect);
    cyw43_arch_lwip_end();
...
bool send_request(struct altcp_pcb *pcb, const char *request, size_t len) {
    ...
    if (lwip_err == ERR_OK) {
        ...
        if (lwip_err == ERR_OK) {
            ...
            while (!((struct altcp_callback_arg *) (pcb->arg))->acknowledged)
                sleep_ms(PICOHTTPS_HTTP_RESPONSE_POLL_INTERVAL);
            if (((struct altcp_callback_arg *) (pcb->arg))->acknowledged !=
                (LEN(request) - 1))
                lwip_err = -1;
        }
    }
